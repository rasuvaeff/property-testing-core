...
- **Every engine change is gated by both adapters' suites.** The
  ...
  `0.2.1` because the Testo adapter constrains core with `^0.2.1`: when core moves to
  ...
  for adapter in testo phpunit; do
    docker run --rm -v "$PWD":/repo -w "/repo/property-testing-$adapter" composer:2 sh -c '
        ...
        trap cleanup EXIT
        composer config repositories.core "{\"type\":\"path\",\"url\":\"../property-testing-core\",\"options\":{\"versions\":{\"rasuvaeff/property-testing-core\":\"0.2.1\"}}}"
        composer update
        composer test
