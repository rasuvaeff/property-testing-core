...
девять кандидатов. Контрпример несёт принятые шаги, поэтому повторный прогон
может пройти по ним, а не искать их заново:
...
с выключенным shrinking, с wall-clock бюджетом спуска, с `maxShrinks` меньше
длины пути, с некорректным путём — отвергаются в конструкторе.
