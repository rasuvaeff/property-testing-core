...
| [`rasuvaeff/property-testing-names`](https://github.com/rasuvaeff/property-testing-names) | Your inputs are people: forms, profiles, auth, validators, reports — `Names::first()`/`last()`/`middle()`/`full()`/`person()` over bundled `en` and `ru` datasets, gender-consistent, shrinking to the shortest entry |
