...
| `targeted_search.php` | Targeted search: a corner bug (three parameters whose sum exceeds a threshold 0.17% of the space reaches) missed by 300 random runs and found by 200 random plus 100 search runs climbing `Target::maximize('sum', …)`; a listener narrates every `TargetImproved` | No |
| `rule_machine.php` | The rule-based stateful façade: a `StackMachine` with `#[Rule]` steps, a `#[Precondition]` guard and an `#[Invariant]`, run through `Gen::rules()` against a correct stack (passes) and a FIFO one (falsified, shrunk to `[push(value: 0), push(value: 1), pop()]`) | No |
