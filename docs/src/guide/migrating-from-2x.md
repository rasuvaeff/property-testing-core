---
title: Migrating from property-testing 2.x
description: "One composer command for Testo; a mapping table for a custom harness; why PHPUnit uses a trait instead of an attribute."
---
...
The same code now ships as three packages — the engine plus one adapter per test
...
- **`composer remove` must come first.** The engine declares
...
<code v-pre>if: ${{ !cancelled() }}</code> — works identically, because the corpus format and
the variable are the same. The full recipe is on
[Regression corpus](/guide/regression-corpus).
...
No test framework comes with it. Build a
[`PropertyDefinition`](/api/classes/Runner/PropertyDefinition), hand it to
[`PropertyRunner`](/api/classes/Runner/PropertyRunner) through a
[`TrialExecutor`](/api/classes/Runner/TrialExecutor), and inspect the structured
[`PropertyResult`](/api/classes/Runner/PropertyResult) — see
[Examples](/guide/examples) and
...
message, with the engine exception as `previous`. See
[PHPUnit adapter](/adapters/phpunit) for the full fluent chain.
