...
| `standalone_runner.php` | Driving the framework-agnostic engine directly: a hand-built `PropertyDefinition`, `CallableTrialExecutor`, structured `PropertyResult` inspection, and the run knobs — `ShrinkMode::Off` reporting a counterexample as generated, `path` replaying a recorded descent (and refusing a stale one), and `phases: [Phase::Examples, Phase::Corpus]` as a fast gate that never reaches the random phase | No |
