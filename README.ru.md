...
`failure` (`{type, message}` или null), `discards`, `skips`, `edgeCases`. Как и
ключи `DistributionReport::toArray()`: `attempts`, `discards`,
`discardPercent`, `skips`, `checks`, `coverageAssessed`, `labels` (каждый —
`{label, count, percent, required, met}`). Ключ может быть добавлен в конец
минором; переименование или удаление — только мажор.
...
| `Gen::uniqueArrayOf($element, $minSize, $maxSize)` | `UniqueArrayArbitrary`, списки попарно различных элементов (строгое `===`, поэтому `NAN` никогда не равен себе и может встретиться несколько раз) | как `arrayOf`, но кандидаты, совпадающие с другим элементом, пропускаются |
...
| `Gen::swarm($arbitrary)` | `SwarmArbitrary`, swarm-тестирование: каждый случай видит лишь непустое подмножество вариантов обёрнутого генератора выбора (`oneOf`, `elements`, `frequency`, `commands`) | внутри подмножества, из которого случай получился, — обратно до полного алфавита не расширяется |
...
| `CorpusFailed` | Корпус бросил исключение (Redis недоступен, ошибка клиента, незавершённая запись на диск); property продолжила без него до конца прогона |
...
`shrunkArguments`, `shrinkSteps`, `shrinkTrials`, `discards`, `skips` и исходный
