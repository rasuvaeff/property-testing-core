...
Skip'ы окружения считаются и ограничиваются отдельно, тем же порогом: о входе
они не говорят ничего, поэтому исчерпание их бюджета сообщает про окружение,
а не про генераторы.
...
| `maxDiscards` | `null` | Бюджет discard'ов и, отдельно, бюджет skip'ов; null = `runs * 10` |
