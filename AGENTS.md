...
- **Every engine change is gated by both adapters' suites.** The
  ...
  `0.12.0` because both adapters accept `^0.12`: when core moves to a version
  ...
  **`docs/.api-workspace/composer.json` is the same class of pin and is part of
  ...
  for adapter in testo phpunit; do
    docker run --rm -v "$PWD":/repo -w "/repo/property-testing-$adapter" composer:2 sh -c '
        ...
        trap cleanup EXIT
        composer config repositories.core "{\"type\":\"path\",\"url\":\"../property-testing-core\",\"options\":{\"versions\":{\"rasuvaeff/property-testing-core\":\"0.12.0\"}}}"
        composer update
        composer test
