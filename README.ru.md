...
| `Gen::floatBetween($min, $max)` | `FloatArbitrary`, `[$min, $max)` — сам `$max` никогда не выпадает | к `0.0`, в пределах диапазона |
...
| 7 | **Семейство** | Адаптеры (`-testo`, `-phpunit`) и `-names` требуют движок каретой на текущий мажор. Мажор здесь — мажор там, и движок выпускается первым. |
| 8 | **PHP** | `8.3 - 8.5`. Поддержка нового минора PHP — патч, расширяющий constraint; снятие версии PHP — мажор. |
